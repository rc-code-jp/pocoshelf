...
- **AI エージェントとの並走がしやすい**
  - 「エージェントが触ったファイル」を自分の目で素早く確認できます。
- **変更点チェックが気軽**
  - Git の変更は“ステータスを読む”より、色でざっくり把握できます。
- **共有しやすいパス形式**
  - `y` で `@path/to/file` をコピーでき、チャットや Issue にそのまま貼れます。
- **ターミナル完結で軽い**
  - エディタを開き直さずに、ツリー確認とテキスト閲覧を行き来できます。
